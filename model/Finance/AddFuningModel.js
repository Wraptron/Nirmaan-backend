...
const AddFundingModel = async (startup_name, funding_type, amount, purpose, funding_date, reference_number, document, description) => {    
    return new Promise((resolve, reject) => {
        client.query(
            "INSERT INTO update_funding(startup_name, funding_type, amount, purpose, funding_date, reference_number, document, description, status) VALUES($1, $2, $3, $4, $5, $6, $7, $8, $9)", 
            [startup_name, funding_type, amount, purpose, funding_date, reference_number, document, description, 'Pending'], 
            (err, result) => {
                if (err) {
                    reject({ err });
                } else {
